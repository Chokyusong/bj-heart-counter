...
st.title("BJ별 하트 정리 자동화")
st.caption("단일 파일 → 관리자용/BJ용 ZIP (합산), 여러 파일 → 총합산 엑셀 (요약 + 참여BJ별 건별 내역)")
...
def visual_len(val) -> int:
    ...
    for ch in s:
        if unicodedata.east_asian_width(ch) in ("F", "W", "A"):
            w += 2
        elif ord(ch) >= 0x1F300:  # emoji rough range
            w += 2
        else:
            w += 1
    return w
...
def normalize_nick(nick: str) -> str:
    """후원자 닉네임: [접두] 및 (괄호내용) 제거"""
    if not isinstance(nick, str):
        return ""
    nick = re.sub(r'^\[.*?\]', '', nick)   # [직책]/[병가] 등 제거
    nick = re.sub(r'\(.*?\)', '', nick)    # (설명) 제거
    return nick.strip()

def normalize_bj(name: str) -> str:
    """참여BJ 표기: [접두] 제거 (예: [병가]치유 → 치유)"""
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    """단일 파일 처리용: ID/닉네임 분리 + 닉네임 정규화 후 참여BJ×ID×닉네임 합산"""
...
def make_bj_excel(bj_name: str, sub_df: pd.DataFrame, admin: bool) -> bytes:
    """관리자용=True: 구분/합계 열 포함, BJ용=False: 간단 표"""
    sub = sub_df.copy()
...
def extract_date_from_name(name: str) -> str:
    """파일명에서 날짜 추출 → YYYY-MM-DD (지원: YYYYMMDD, YY-MM-DD, MMDD 등)"""
    s = name.lower()
...
def build_master_excel_bytes(merged_df, df_daily, df_total) -> bytes:
    """총합산 엑셀: 요약 2시트 + 참여BJ(정규화)별 시트(건별 내역 + 최상단 총합)"""
    bio = io.BytesIO()
    with pd.ExcelWriter(bio, engine="openpyxl") as w:
        # 1) 요약 시트 2개
...
if uploaded:
    try:
        ...
        with left:
            ...
            for i, (fname, data) in enumerate(sorted(admin_files.items())):
                st.download_button(f"⬇️ {fname}", data=data, file_name=fname,
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True, key=f"admin-{i}-{fname}")
        ...
        with right:
            ...
            st.divider(); st.markdown("**개별 파일 다운로드**")
            for i, (fname, data) in enumerate(sorted(bj_files.items())):
                st.download_button(f"⬇️ {fname}", data=data, file_name=fname,
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True, key=f"bj-{i}-{fname}")
...
if multi:
    ...
    if all_rows:
        ...
        if not need_cols.issubset(set(merged.columns)):
            ...
            st.subheader("요약_일별")
            st.dataframe(daily_out, use_container_width=True, hide_index=True)

            # === 요약_참여BJ_총계 (참여BJ 정규화 적용!) ===
